 ...
 * @since    1.0
 */
if ( ! function_exists( 'wr_ferado_schema_metadata' ) ) :
	/**
	 ...
	 * @return  void
	 */
	function wr_ferado_schema_metadata( $args ) {
		if ( ! empty( $args ) ) {
			$args = array_merge(
				array(
					'post_type' => '',
		...
		$args = apply_filters( 'wr_ferado_schema_metadata_args', $args );
		...
		$attributes = apply_filters( 'wr_ferado_schema_metadata_attributes', $attributes, $args );
		...
		$markup = apply_filters( 'wr_ferado_schema_metadata_output', $markup, $args );
